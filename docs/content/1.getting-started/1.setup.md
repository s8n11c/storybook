...
1. Add `s8n11c-nuxtjs-storybook` as a development dependency to your project:
...
```bash [pnpm]
pnpm i -D s8n11c-nuxtjs-storybook
```

```bash [yarn]
yarn add -D s8n11c-nuxtjs-storybook
```

```bash [npm]
npm install -D s8n11c-nuxtjs-storybook
```

::

2. If not using `nuxi` above, add the module `s8n11c-nuxtjs-storybook` to your Nuxt configuration:
...
- Modify the `s8n11c-nuxtjs-storybook` dependency in your `package.json` to use the latest nightly release:

  ```json
  {
    "devDependencies": {
      "s8n11c-nuxtjs-storybook": "npm:s8n11c-nuxtjs-storybook@nightly"
    }
  }
  ```
...
- Change the `s8n11c-nuxtjs-storybook` dependency in `package.json` back to the stable version:

  ```json
  {
    "devDependencies": {
      "s8n11c-nuxtjs-storybook": "latest"
    }
  }
  ```
